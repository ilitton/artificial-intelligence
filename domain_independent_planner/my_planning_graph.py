...
def mutexify(node1: PgNode, node2: PgNode):
    """ adds sibling nodes to each other's mutual exclusion (mutex) set. These should be sibling
    nodes!
...
class PlanningGraph():
    ...
    def __init__(self, problem: Problem, state: str, serial_planning=True):
        ...
        Instance variable calculated:
            ...
            all_actions: list of the PlanningProblem valid ground actions combined with calculated
                no-op actions
            s_levels: list of sets of PgNode_s, where each set in the list represents an S-level in 
                the planning graph
            a_levels: list of sets of PgNode_a, where each set in the list represents an A-level in 
                the planning graph
        """
    ...
    def create_graph(self):
        ...
        start of a problem planning search, this will be the same as the initial state of the 
        problem. However, the planning graph can be built from any state in the Planning Problem

        This function should only be called by the class constructor.

        :return:
            builds the graph by filling s_levels[] and a_levels[] lists with node sets for each 
            level
        """
    ...
    def add_action_level(self, level):
        ...
        2. connect the nodes to the previous S literal level
            - For example, the A0 level will iterate through all possible actions for the problem 
                and add a PgNode_a to a_levels[0] set iff all prerequisite literals for the action 
                hold in S0. 
            - This can be accomplished by testing to see if a proposed PgNode_a has prenodes that 
                are a subset of the previous S level.  
            - Once an action node is added, it MUST be connected to the S node instances in the 
                appropriate s_level set.
        
        :param level: int
            the level number alternates S0, A0, S1, A1, S2, .... etc the level number is also used 
            as the index for the node set lists self.a_levels[] and self.s_levels[]
        :return:
            adds A nodes to the current level in self.a_levels[level]
        """
        action_nodes = []
        for action in self.all_actions:
            # check preconditions to determine if node is reachable 
            ...
            if action_node.prenodes.issubset(self.s_levels[level]):
                ...
                for state_node in self.s_levels[level]:
                    if state_node in action_node.prenodes:
                        # connect nodes to previous S literaly level to make it reachable
    ...
    def add_literal_level(self, level):
        ...
        2. connect the nodes
        for example, every A node in the previous level has a list of S nodes in effnodes that 
        represent the effect produced by the action. These literals will all be part of the new S 
        level. Since we are working with sets, they may be "added" to the set without fear of 
        duplication. However, it is important to then correctly create and connect all of the new S 
        nodes as children of all the A nodes that could produce them, and likewise add the A nodes 
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # check if node 2 actions negate node 1 actions
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True
        
        # check if node 1 actions negate node 2 actions
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # check if node 1 actions are negation of node 2 precondition
        for effect in node_a1.action.effect_add:
        ...
        # check if node 2 actions are negation of node 1 precondition
        for effect in node_a2.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # check for mutual exclusivity between node_a1 and node_a2 preconditions
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # check if both are state symbols and negation of each other
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # check for mutual exclusitivity between node_s1 and node_s2 preconditions
